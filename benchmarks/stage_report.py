...
def render(report):
    ...
    lines = ['# Guest startup stage measurement', '',
             f"Source: `{environment.get('commit', 'unknown')}`; platform: {environment.get('platform', 'unknown')}; Python: {environment.get('python', 'unknown')}.", '',
    ...
    lines += ['', '## End-to-end and correlated process observations', '',
              '| Case | DBs | p50 / p95 ms | Median sampled CPU s | Median sampled peak RSS MiB |',
              '| --- | ---: | ---: | ---: | ---: |']
