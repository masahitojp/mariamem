...
binary_minimums = ({p.name: check_binary(p, major) for p in (host, args.runtime)}
                   if major is not None else {})
linux_dependencies = ({p.name: elf_dependencies(p) for p in (host, args.runtime)}
...
manifest = {"version": 1, "package_version": PYTHON_VERSION, **platform_fields(target), "wasmer_version": "7.4.2", "public_release_ready": bool(ready),
