...
func Resolve(dir string) (Bundle, error) {
	...
	if runtime.GOOS != "darwin" || runtime.GOARCH != "arm64" {
		return Bundle{}, diagnostic.Wrap("unsupported_platform", "platform", fmt.Errorf("mariamem requires macOS 15+ arm64 or Ubuntu 24.04 x86_64; got %s-%s", runtime.GOOS, runtime.GOARCH))
	}
...
func ubuntuPlatform(raw string) error {
	...
	if values["ID"] != "ubuntu" || values["VERSION_ID"] != "24.04" {
		return diagnostic.Wrap("unsupported_platform", "platform", fmt.Errorf("requires Ubuntu 24.04 x86_64; got ID=%q VERSION_ID=%q", values["ID"], values["VERSION_ID"]))
	}
	return nil
}
...
func resolve(dir, platform string, major int) (b Bundle, err error) {
	...
	if m.Version != 1 || m.Platform != platform {
		return b, fmt.Errorf("unsupported native bundle version/platform")
	}
	...
	if platform == "ubuntu24.04-x86_64" && (m.Distribution != "ubuntu" || m.VersionID != "24.04" || m.Architecture != "x86_64") {
		return b, fmt.Errorf("native bundle requires Ubuntu 24.04 x86_64 metadata")
	}
